  ...
  async _check(key, alias = {}) {
    ...
    if (isCustomFunc) {
      try {
        ...
        result = new RuleResult(false, error.msg || error.message || 'Parameter error')
      }
      // 函数验证
    } else {
      // 属性验证, 数组，内有一组Rule
...
class Rule {
  ...
  validate(field) {
    ...
    if (!validator[this.name](field + '', ...this.params)) {
      return new RuleResult(false, this.msg || this.message || 'Parameter error')
    }
  ...
  validate(field) {
    if (field == null) {
      ...
      if (allowEmpty) {
        ...
        return new RuleFieldResult(false, 'This field is a required parameter')
      }
    }
